...
namespace ArcGISRuntimeXamarin.Samples.NavigateAR
{
    public partial class RouteViewer : ContentPage, IDisposable
    {
        ...
        private void CalibrateButton_Clicked(object sender, EventArgs e) { 
            ...
            if (IsCalibrating)
            {
                CalibrateButton.BackgroundColor = Color.DarkSlateBlue;
            } else 
            {
                CalibrateButton.BackgroundColor = Color.LightSkyBlue;
            }
        ...
        private void RouteTracker_VoiceGuidanceChanged(object sender, RouteTrackerNewVoiceGuidanceEventArgs e)
        {
            Device.BeginInvokeOnMainThread(() =>
            {
                ...
                string initialTextHelp = e.VoiceGuidance.Text;
                // Best traslate in help label
                // DisplayAlert("Mesaje", "datos: " + initialTextHelp, "Ok");
                if (initialTextHelp.Substring(1, 3) == "In ")
                {
                    initialTextHelp = "En " + initialTextHelp.Substring(4, initialTextHelp.Length);
                }
                if (initialTextHelp.Contains("meters"))
                {
                    initialTextHelp.Replace("meters", "metros");
                }
                
                HelpLabel.Text = initialTextHelp;
            });
