...
namespace ArcGISRuntime
{
    public partial class CategoryListPage
    {
        ...
        private async void Initialize()
        {
            ...
            bool centrosSaludOK = await App.SQLiteDB.SaveMulitCentroSalud();

            if (!centrosSaludOK) {
                await Application.Current.MainPage.DisplayAlert("Mensaje", "Los centros de salud no se cargaron.", "OK");
            }
        ...
        private async void OnItemTapped(object sender, ItemTappedEventArgs e)
        {
            ...
            if (category != null)
            {
                ...
                if (category.Name == "Realidad Aumentada")
                {
                    bool arCompatible = await ViewModel.CheckARAndroid();
                    if (!arCompatible)
                    {
                        // Inform user AR is not supported.
                        await Application.Current.MainPage.DisplayAlert("Realidad Aumentada not supported", "Camera permissions are required for use of augmented reality.", "OK");
    ...
    public class SamplesSearchViewModel : INotifyPropertyChanged
    {
        ...
        public async Task<bool> CheckARAndroid()
        {
            ...
            if (!arCompatible)
            {
                SampleCategories.RemoveAll(category => category.Name == "Realidad Aumentada");
                _allSamples.RemoveAll(sample => sample.Category == "Realidad Aumentada");
